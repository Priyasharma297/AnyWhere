...
const Service = () => {
  ...
  const servicee = [
    {
      ...
      img: "./Images/mech.png", // Placeholder image
    },
    {
      id: 2,
      name: "Jane Smith",
      address: "456 Elm St, City, Country",
      img: "./Images/mech2.png", // Placeholder image
    },
    {
      id: 3,
      name: "Mike Johnson",
      address: "789 Oak St, City, Country",
      img: "./Images/mech3.webp", // Placeholder image
    },
  ];
  ...
  return (
    <div>
      ...
      {!isSignedIn ? (
        <div>
          <div className=" hidden sm:block w-full bg-gray-200 p-4 mb-8 rounded-lg">
            <div className="w-1/2 mx-auto flex justify-between items-center relative">
            <div className="hidden sm:block absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full h-1 rounded-full bg-gray-300">
  <div
    className={`absolute top-0 left-0 h-full rounded-full ${
      tier && brand && model && servic
        ? 'w-full sm:w-[90%] bg-gray-500'
        : tier && brand && model
        ? 'w-4/5 sm:w-[75%] bg-gray-500'
        : tier && brand
        ? 'w-3/4 sm:w-[65%] bg-gray-500'
        : tier
        ? 'w-2/3 sm:w-[55%] bg-gray-500'
        : 'w-0'
    }`}
  />
</div>
              ...
              <div className="relative z-10 flex flex-col items-center">
                <div
                  className={`w-20 h-20 flex items-center justify-center rounded-full text-em font-bold ${"bg-gray-400"}`}
                >
                  Vehicle
                </div>
              </div>
              ...
              <div className="relative z-10 flex flex-col items-center">
                <div
                  className={`w-20 h-20 flex items-center justify-center rounded-full text-em font-bold ${
                    tier && brand ? "bg-gray-400" : "bg-gray-100"
                  }`}
                >
                  Model
                </div>
              ...
              <div className="relative z-10 flex flex-col items-center">
                <div
                  className={`w-20 h-20 flex items-center justify-center rounded-full text-em font-bold ${
                    tier && brand && model ? "bg-gray-400" : "bg-gray-100"
                  }`}
                >
                  Services
                </div>
          ...
          <br></br>
          {!tier && (
            <div className="flex flex-wrap justify-center gap-8 mb-8 sm:flex-col md:flex-row">
            <button
            ...
            >
              <img
                ...
                alt="Two Wheelers"
                className="w-80 h-80 mb-2 sm:w-64 sm:h-64"
              />
              ...
              ? motorcycleBrands.map((brandItem) => (
                  ...
                  >
                    <img
                      src={brandItem.image}
                      alt={brandItem.name}
                      className="w-40 h-40 mb-2 sm:w-32 sm:h-32 md:w-40 md:h-40 lg:w-48 lg:h-48"
                    />
                    <span className="text-sm font-medium text-gray-800 hover:text-gray-600 transition-colors duration-300">
                      {brandItem.name}
                    </span>
          ...
          {brand && !model && (
            <div className="flex flex-wrap justify-center gap-8 mb-8 sm:flex-col md:flex-row">
            {(tier === "Tier 1" ? motorcycleModels[brand] : carModels[brand]).map((modelItem) => (
              <button
                key={modelItem.name}
              ...
              >
                <img
                  src={modelItem.image}
                  alt={modelItem.name}
                  className="w-60 h-60 mb-2 sm:w-48 sm:h-48 md:w-60 md:h-60 lg:w-72 lg:h-72"
                />
                <span className="text-sm font-medium text-gray-800 hover:text-gray-600 transition-colors duration-300">
                  {modelItem.name}
                </span>
